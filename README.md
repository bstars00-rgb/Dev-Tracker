...
**상단 카드 7개** — Total · Live · In flight · First contact · Needs attention · Stalled 90d+ · No activity record.
숫자를 클릭하면 그 조건으로 필터링된 리스트로 넘어갑니다.

**Stage funnel** — 11단계를 몇 건이 통과했고 지금 어디에 몰려 있는지.
각 줄의 `−N` 은 직전 단계 대비 빠진 건수, 오른쪽은 그 단계에 멈춰 있는 건수입니다.
막대를 클릭하면 그 단계에 머무는 건들만 리스트로 넘어갑니다.

**PIC별 부하** — 담당자별 건수와 그중 손이 필요한 건수. 초록 = Live, 빨강 = 위험, 회색 = 그 외.

**카테고리별 현황** — 연동 유형별 분포와 위험 비중.

**마지막 활동 경과** — 진행 중 건들의 침묵 기간 분포 (14일 이내 / 15–30 / 31–90 / 90일 초과 / 기록 없음).

**지금 손봐야 할 10건** — 방치 심각도 → 진행률 → 경과일 순 상위 10건.
