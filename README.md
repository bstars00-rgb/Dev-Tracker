...
**카드 8장** — 숫자만 있는 게 아니라 각각 네 가지를 담습니다: 값, 전체 대비 비중, 구성 막대, 한 줄 해설.

| 카드 | 해설에 뜨는 것 |
|---|---|
| 전체 | 최다 상태와 건수 |
| 완료 (Live) | **단계 날짜로 계산한 평균 소요일** |
| 진행 중 | 가장 앞선 단계 |
| 초기 접촉 | 전체 대비 비율 |
| 손이 필요함 | **가장 오래 방치된 건 이름과 일수** |
| 90일+ 방치 | 최장기 건 이름과 일수 |
| 활동 기록 없음 | PIC 미지정 건수 |
| High Impact | 그중 위험 건수 |

구성 막대에 마우스를 올리면 각 구간의 건수가 뜹니다. 카드를 클릭하면 해당 조건으로 필터된 목록으로 넘어갑니다.

**PIC별 부하** — 담당자별 건수와 위험 건수. 초록 = 완료, 빨강 = 위험, 회색 = 그 외.
**카테고리별 현황** — 연동 유형별 분포와 위험 비중.
**마지막 활동 경과** — 진행 중 건들의 침묵 기간 분포.
**지금 손봐야 할 10건** — 방치 심각도 → 진행률 → 경과일 순.

전부 클릭하면 목록으로 드릴다운됩니다.
